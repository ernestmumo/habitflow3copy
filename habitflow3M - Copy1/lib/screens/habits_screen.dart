                  ...
                  SlidableAction(
                    onPressed: (context) {
                      Get.defaultDialog(
                        ...
                        onConfirm: () {
                          controller.deleteHabit(int.parse(habit.id));
                          Get.back();
                        },
                      );
                    },
                    ...
                    child: Row(
                      children: [
                        ...
                        Obx(() {
                          ...
                          final habitDate = DateTime(now.year, now.month, now.day); // Habits list is always for today
                          final isToday = habitDate.isAtSameMomentAs(today);
                          return Checkbox(
                            activeColor: Colors.blue.shade600,
                            value: habit.isDone.value,
                            onChanged: isToday ? (value) {
                              controller.toggleHabitCompletion(int.parse(habit.id));
                              // Also refresh calendar so UI stays in sync
                              final calendarController = Get.find<CalendarController>();
                              final userController = Get.find<UserController>();
                              final userId = userController.currentUserId;
                              if (userId != null) {
                                calendarController.loadAllCompletions(userId);
                              } else {
                                // Handle case where user might be logged out somehow?
                                debugPrint('[HabitsScreen] Cannot refresh calendar, no user logged in.');
                              }
                            } : null,
                          );
                        }),
    ...
    Get.dialog(
      AlertDialog(
        ...
        actions: [
          ...
          Obx(() => ElevatedButton(
            onPressed: controller.isLoading.value ? null : () async { // Make async and disable if loading
              // Validate inputs
              ...
              // Add habit to controller and wait for completion
              await controller.addHabit(newHabit);
              
              // Check if still mounted before interacting with context/Get
              if (!context.mounted) return; 

              // Close dialog and show success message only after completion
              Get.back(); 
              Get.snackbar(
                'Success',
            ...
            child: controller.isLoading.value 
                   ? SizedBox( // Show loading indicator inside button
                       width: 20, 
                       height: 20, 
